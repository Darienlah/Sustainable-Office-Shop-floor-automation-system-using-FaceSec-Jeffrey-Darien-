            ...
            if sync_time > script_start_time:
                ...
                if 'accessRecordId' in data:
                    ...
                    if float(point2) < 0.8:
                        ...
                        wake_on_lan('80:00:0B:44:8E:69', '192.168.0.101') # when running on new pc, add mac address and ip of the pc
                        ...
                        r = requests.post("http://192.168.0.101:5000", data=name) # when running on new pc, add ip of the new pc
...
async def main():
    ...
    async with websockets.serve(handler, "192.168.0.104", 8000):
